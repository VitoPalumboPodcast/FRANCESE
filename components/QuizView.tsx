...
import { Loader2, Check, X, Award, ArrowLeft, Volume2, Flame, Sparkles, RotateCcw, Clock, Zap, Brain, Utensils, Train, Bike, Plane, Heart, Eye, Home, Camera, Landmark, Music, Sun, Coffee, ShoppingBag, Users, Smile, Languages } from 'lucide-react';
...
const QuizView: React.FC<QuizViewProps> = ({ topicId, onBack }) => {
  ...
  const playAudio = (text: string, lang: 'fr-FR' | 'it-IT' = 'fr-FR') => {
  ...
  const toggleTranslation = (e: React.MouseEvent) => {
      ...
      if (newState && questions[currentQIndex]?.questionTranslation) {
          playAudio(questions[currentQIndex].questionTranslation, 'it-IT');
      }
  };
      ...
      <div className="flex-grow flex flex-col justify-center">
        ...
        <div className="mb-10 text-center relative group">
            ...
            {isCloze ? (
                ...
                <h3 className="text-2xl md:text-3xl font-serif font-medium text-slate-800 mb-2 leading-tight text-center">
                  {currentQ.question}
                </h3>
            )}
            
            {/* Translation & Audio Help Button */}
            <div className="mt-4 flex flex-col items-center justify-center">
                 ...
                 >
                    <Languages size={14} />
                    {showTranslation ? 'Nascondi Traduzione' : 'Traduci'}
                 </button>
                 
                 {showTranslation && currentQ.questionTranslation && (
                     <div className="mt-2 text-slate-500 italic font-medium animate-in slide-in-from-top-2">
                        {currentQ.questionTranslation}
                     </div>
                 )}
            </div>
